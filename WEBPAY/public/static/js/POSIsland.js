...
// src/shared/utils/QRCodeUtility.ts
var QRCode = __toESM(require_browser());
var QRCodeUtility = {
  draw(canvas, url) {
    void QRCode.toCanvas(canvas, url, { errorCorrectionLevel: "H", width: 256, margin: 1 }).catch((err) => console.error("QR render failed:", err));
  }
};

// src/client/islands/POSIsland.ts
var POSIsland = (() => {
  let currentDrawerType = null;
  let currentDrawerId = null;
  let countdownIntervalId = null;
  let countdownDeadlineMs = null;
  function handleHydration(hydration) {
    ...
    if (newInvoiceBtn) newInvoiceBtn.addEventListener("click", handleNewSaleClick);
    document.addEventListener("invoice:terminal", () => {
      stopCountdown();
      showExpiredQRMessage();
      purgeInvoiceFromPage();
    });
    if (hydration.magicLink) drawQRCode();
  }
  function showExpiredQRMessage() {
    const qrCanvas = document.getElementById("qr");
    if (!qrCanvas) return;
    qrCanvas.style.display = "none";
    let msg = document.getElementById("qr-expired-msg");
    if (!msg) {
  ...
  function removeExpiredQRMessage() {
    let msg = document.getElementById("qr-expired-msg");
    if (!msg) return;
  ...
  function handleCopyLinkClick(event) {
    ...
    const magicLink = window.__PAGE__?.origLink;
    if (!magicLink) return;
  ...
  function handleNewSaleClick(event) {
    event.preventDefault();
    resetPOSUI();
    purgeInvoiceFromPage();
  ...
  function handleFormSubmit(form, event) {
    ...
    const storeId = window.__PAGE__?.storeId;
    ...
    fetch(`/merchant/stores/${storeId}/prepare-invoice`, {
      ...
      window.__PAGE__.magicLink = dto.magicLink;
      window.__PAGE__.origLink = dto.origLink;
      if (dto.invoiceId) {
        window.__PAGE__.invoiceId = dto.invoiceId;
        document.dispatchEvent(new Event("invoice:ready"));
      }
      removeExpiredQRMessage();
      drawQRCode();
      showPaymentCard(dto);
      const ttlSeconds = typeof dto.ttl === "number" ? dto.ttl : typeof dto.expiresIn === "number" ? dto.expiresIn : ttl;
  ...
  function startCountdown(ttlSeconds) {
    stopCountdown();
    countdownDeadlineMs = Date.now() + Math.max(0, Math.floor(ttlSeconds)) * 1e3;
    updateCountdownDom();
    countdownIntervalId = window.setInterval(() => {
      const remaining = countdownDeadlineMs - Date.now();
      if (remaining <= 0) {
        stopCountdown();
        showExpiredQRMessage();
        purgeInvoiceFromPage();
  ...
  function revealNewInvoiceAction() {
    const btn = document.getElementById("new-invoice");
    if (btn) btn.style.display = "";
  }
  function hideNewInvoiceAction() {
    const btn = document.getElementById("new-invoice");
    if (btn) btn.style.display = "none";
  }
  function purgeInvoiceFromPage() {
    if (window.__PAGE__) {
      delete window.__PAGE__.invoiceId;
      delete window.__PAGE__.magicLink;
    }
    const qrCanvas = document.getElementById("qr");
    if (qrCanvas) {
  ...
  function handleSubscriptionRowClick(event) {
    event.preventDefault();
    const row = event.currentTarget;
  ...
  function drawQRCode() {
    const qrCanvas = document.getElementById("qr");
    const magicLink = window.__PAGE__?.magicLink;
  ...
  function enablePOSUI() {
    const posRoot = document.getElementById("pos-root");
    posRoot?.removeAttribute("aria-disabled");
    enablePOSForm();
  }
  function disablePOSUI(message) {
    const posRoot = document.getElementById("pos-root");
    posRoot?.setAttribute("aria-disabled", "true");
    showPOSFormError(message);
    disablePOSForm();
  }
  function disablePOSForm(message) {
    const form = document.getElementById("new-sale-form");
    form.querySelectorAll("input,button").forEach((el) => el.disabled = true);
    if (message) showPOSFormError(message);
  }
  function enablePOSForm() {
    const form = document.getElementById("new-sale-form");
    form.querySelectorAll("input,button").forEach((el) => el.disabled = false);
    hidePOSFormError();
  }
  function clearPOSFormFields() {
    const form = document.getElementById("new-sale-form");
    form.reset();
    const amount = form.querySelector('input[name="amount"]');
    const ttl = form.querySelector('input[name="ttl"]');
    const memo = form.querySelector('input[name="memo"]');
  ...
  function buildDefaultMemo() {
    const form = document.getElementById("new-sale-form");
    const brandingNameCnt = form ? form.querySelector('input[name="displayNameField"]') : null;
...
//# sourceMappingURL=POSIsland.js.map
