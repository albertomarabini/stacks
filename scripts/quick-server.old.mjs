...
results.push(await step("merchant: prepare-invoice returns dto+unsigned+deeplink", async () => {
  try {
    const body = {
      amount_sats: 18000,
      ttl_seconds: 600,
      memo: "prep-onecall",
      payerPrincipal: PAYER_ADDR,  // used to scope PCs in builder
    };
    ...
    // 3) deeplink present and looks like StacksPay (URI scheme web+stx:stxpay1…)
    const deeplink = String(j?.stacksPayURI || "");
    const okDeeplink = /^web\+stx:stxpay1/i.test(deeplink);

    // 4) magicLink present (public fallback)
    const okMagic = typeof j?.magicLink === "string";

    // Optional: sanity-read the magic link payload
    ...
    // log for debugging like other steps
    if (okUnsigned) logUnsigned("prepare-invoice unsigned", j.unsignedCall);
    try {
        const { validateStacksPayURL } = require("stacks-pay");
        const v = validateStacksPayURL(String(j.stacksPayURI));
        if (!v?.isValid) return fail("stacksPayURI invalid", v);
      } catch { /* optional dep; ignore if missing */ }
    return okDto && okUnsigned && okPcs && okDeeplink && okMagic && okMagicFetch;
  } catch (e) {
    // If route not implemented yet, don’t hard-fail the suite
    return (e.status === 404 || e.status === 405)
      ? skip("merchant: prepare-invoice returns dto+unsigned+deeplink", "route missing")
...
results.push(await step("merchant: sub direct create-tx ok (or 409 until due)", async () => {
 ...
 // PASS on: 409 (server pre-gate), chain u207, or 400 NoSuchContract (ALT not deployed)
results.push(await step("subscription wrong-token blocked (u207)", async () => {
    ...
    try {
      ...
      // server correctly blocks when not due → call is blocked before we can tamper: count as PASS
      if (is409BadStatus(e)) return true;
    ...
    // tamper last arg (token contract principal)
    const fake = { type: "contract", value: ALT };
    const tampered = { ...unsigned, functionArgs: unsigned.functionArgs.slice(0, -1).concat(fake) };
    ...
    try {
      ...
      // If ALT points to a non-existent contract, node rejects before contract runs → also OK
  ...
  // try until due, else SKIP (don’t fail the suite)
  results.push(await step("subscription pay at/after due succeeds & bumps next-due", async () => {
    await setDirectMode(sub.id);
    const deadline = Date.now() + (MAX_WAIT_MS * 2);
    while (Date.now() < deadline) {
      try {
        ...
        const { txid } = await signAndBroadcastUnsigned(unsigned, PAYER_SK);
        const st = await waitForFinal(txid);
        if (st?.tx_status === "success") return true;
      } catch (e) {
        if (!is409BadStatus(e)) console.warn("pay-at/after-due:", errStatus(e), errBody(e) || e?.message);
      }
      await sleep(1200);
    }
    return skip("subscription pay at/after due", "never reached due within timeout");
  }, [need.storeId(), need.apiKey(), need.subId(), need.payerSigner(), need.payReady()]));
