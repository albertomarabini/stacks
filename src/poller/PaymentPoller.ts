...
export class PaymentPoller {
  ...
  async onInvoiceCanceled(event: NormalizedEvent): Promise<void> {
    // delegate to invoice applier to mark canceled and dispatch webhooks
    if (!this.invoiceApplier || !this.dispatcher) return;
    await this.invoiceApplier.handleCanceled(event);
  }
